...
\begin{enumerate}
    \item \textbf{Rejection Sampling Failure:} Across $2,000,000$ generated planetary states, the acceptance rate under evidence $\mathbf{e}$ was only $8.72\%$. More than $91\%$ of all generated samples were discarded. Zero CFC detections occurred in accepted samples, demonstrating that conditioning on real telescope evidence compounds the rarity bottleneck.
    \item \textbf{MCMC Rare-State Trap:} In a Metropolis-Hastings run of $100,000$ steps (burn-in $2,000$, global acceptance rate $30.8\%$), the biomarker DMS ($X_7$) was visited $1,472$ times ($P_{\mathrm{est}} \approx 0.0147$), while the technosignature CFC ($X_8$) recorded zero visits ($0$ hits), confirming the mode entrapment predicted by Kac's return theorem \eqref{eq:kac_return_theorem}.
