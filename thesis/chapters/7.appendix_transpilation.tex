...
\begin{itemize}[leftmargin=*]
    ...
    \begin{itemize}
        \item Transpiled Depth: $4$
        \item Total Gate Count (Size): $6$
        \item Gate Breakdown: $3\times\mathrm{CRY}$, $1\times\mathrm{U}_3$, $1\times\mathrm{RY}$, $1\times\mathrm{X}$.
    \end{itemize}
    ...
    \begin{itemize}
        \item Transpiled Depth: $9$
        \item Total Gate Count (Size): $30$
        \item Gate Breakdown: $16\times\mathrm{X}$, $6\times\mathrm{CRY}$, $4\times\mathrm{U}_3$, $2\times\mathrm{U}_2$, $1\times\mathrm{Z}$, $1\times\mathrm{MCX}$.
    \end{itemize}
...
Transpilation of the full $17$-qubit circuit yields an unrolled circuit depth of $\mathbf{43{,}874}$ and a total gate volume of $\mathbf{44{,}853}$ operations. Table \ref{tab:master_qae_gates} details the complete, exact gate inventory extracted from the compiler pass.
...
\midrule
Multi-Controlled Phase & \texttt{mcphase} & $15{,}283$ & $34.073\%$ \\
Toffoli (Controlled-Controlled-NOT) & \texttt{ccx} & $14{,}136$ & $31.516\%$ \\
Single-Qubit General Rotation & \texttt{u2} & $8{,}680$ & $19.352\%$ \\
Controlled-NOT (CNOT) & \texttt{cx} & $5{,}115$ & $11.404\%$ \\
Controlled-$R_Z$ Rotation & \texttt{crz} & $1{,}054$ & $2.350\%$ \\
Controlled-Unitary & \texttt{cu} & $496$ & $1.106\%$ \\
Hadamard & \texttt{h} & $72$ & $0.161\%$ \\
Controlled-Phase & \texttt{cp} & $10$ & $0.022\%$ \\
Projective Measurement & \texttt{measure} & $5$ & $0.011\%$ \\
Qubit Swap & \texttt{swap} & $2$ & $0.004\%$ \\
\midrule
\textbf{Total Operations} & & $\mathbf{44{,}853}$ & $\mathbf{100.000\%}$ \\
\bottomrule
\end{tabular}
\end{table}

\notatfm{Multi-qubit controlled gates (\texttt{mcphase}, \texttt{ccx}, \texttt{cx}) constitute $>77\%$ of the master circuit volume.}
As shown in Table \ref{tab:master_qae_gates}, multi-controlled phase gates (\texttt{mcphase}, $34.07\%$) and Toffoli gates (\texttt{ccx}, $31.52\%$) dominate the compiled instruction stream. This concentration arises directly from the sequential conditioning of the 9-node DAG and the multi-qubit reflection operator $S_0 = 2\ket{0}\bra{0} - I$, which requires high-order entanglement across the state and ancilla registers.

\subsection{Native Two-Qubit CNOT Decomposition Overhead}\label{subsec:app_cnot_decomposition_overhead}
It is important to distinguish between the abstract gate counts reported in Table \ref{tab:master_qae_gates} (which retain multi-controlled unitary instructions in the universal basis $\mathcal{B}_{\mathrm{target}}$) and the fully decomposed physical CNOT depth. Standard unitary synthesis of a 3-qubit Toffoli gate (\texttt{ccx}) requires at least 6 CNOT gates and 7 single-qubit rotations \cite{barenco1995}. Consequently, unrolling the $14{,}136$ Toffoli operations alone contributes $14{,}136 \times 6 = 84{,}816$ CNOT operations to the physical execution graph, while decomposing the $15{,}283$ multi-controlled phase gates (\texttt{mcphase}) introduces further entangling layers. 

This brings the total native two-qubit CNOT count well beyond $85{,}000$ operations when compiled without auxiliary borrowed-ancilla networks. This substantial compilation footprint provides the empirical justification for classifying canonical QAE as an exclusive Fault-Tolerant Quantum Computing (FTQC) blueprint, directly motivating the transition to Iterative QAE (IQAE) for near-term physical QPU viability.
...
Table \ref{tab:qae_scaling_registers} compiles the empirical and projected hardware resource requirements as the evaluation precision increases from $n_E = 3$ to $n_E = 10$. The projected gate volumes follow the analytical scaling model $\text{Estimated Gates} \approx N_Q \times 1{,}415 = (2^{n_E} - 1) \times 1{,}415$, where each controlled Grover iterate accounts for approximately $1{,}415$ compiled physical instructions. For the baseline $n_E = 5$, this closed-form model yields $43{,}865$ operations, matching the empirically transpiled master circuit total of $44{,}853$ within $2.2\%$ (with the residual difference attributable to the $\mathrm{IQFT}^\dagger$ permutation network and classical readout measurement operations).
...
\midrule
$3$ & $7$ & $0.392699$ & $0.146447$ & $9{,}905$ \\
$4$ & $15$ & $0.196350$ & $0.038060$ & $21{,}225$ \\
$\mathbf{5}$ & $\mathbf{31}$ & $\mathbf{0.098175}$ & $\mathbf{0.009607}$ & $\mathbf{43{,}865}$ \\
$6$ & $63$ & $0.049087$ & $0.002408$ & $89{,}145$ \\
$7$ & $127$ & $0.024544$ & $0.000602$ & $179{,}705$ \\
$8$ & $255$ & $0.012272$ & $0.000151$ & $360{,}825$ \\
$9$ & $511$ & $0.006136$ & $0.000038$ & $723{,}065$ \\
$10$ & $1{,}023$ & $0.003068$ & $0.000009$ & $1{,}447{,}545$ \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}[leftmargin=*]
    \item At $n_E = 5$ (the baseline implemented in this work), the compiled circuit depth of $43{,}874$ exceeds the coherent threshold of unmitigated NISQ physical devices by more than two orders of magnitude. Consequently, full QPE-based QAE strictly requires fault-tolerant logical qubits supported by surface codes.
