...
\begin{enumerate}
    ...
    \end{equation}
    This achieves quadratic acceleration over classical MCMC ($\mathcal{O}(1/\sqrt{M})$), reducing query complexity from $M_{\mathrm{classical}} \sim 10^8$ to $M_{\mathrm{quantum}} \sim 10^4$ at precision $\epsilon = 10^{-4}$. As derived in Section \ref{sec:critique_quantum_alternatives}, the analytical crossover point occurs at $\epsilon_{\mathrm{cross}} \approx \frac{\pi}{2}\sqrt{p} \approx 1.57 \times 10^{-3}$, beyond which the quantum pipeline outperforms classical sampling by orders of magnitude. However, the master 17-qubit circuit with $n_E = 5$ requires 31 controlled Grover iterations, compiling to over $42,000$ CNOT gates. This circuit constitutes an exact blueprint for early Fault-Tolerant Quantum Computers (FTQC) with logical qubits, but exceeds the physical coherence budget of current hardware.
    
    \item \textbf{The Noisy Intermediate-Scale Quantum Regime (NISQ Proof of Concept):} To establish near-term physical viability, we examined the behavior of an isolated 5-qubit inference kernel ($n_S = 4$ state nodes plus 1 ancilla phase flag, transpiled to depth $D = 39$) under calibrated IBM Quantum noise profiles ($T_1 = 150\,\mu\mathrm{s}$, $T_2 = 120\,\mu\mathrm{s}$, CNOT error $1.20\%$). Unmitigated decoherence degraded the target expectation value from $0.0500$ down to $0.0382$ (a $23.6\%$ error). By applying Zero-Noise Extrapolation (ZNE) via global unitary gate folding ($U \to U(U^\dagger U)^n$ with $\lambda \in \{1, 3, 5\}$) and second-order Richardson extrapolation, we successfully canceled $86.46\%$ of the hardware noise, recovering a mitigated expectation value $E_{\mathrm{ZNE}} = 0.04840$ without physical qubit overhead.
\end{enumerate}
...
\textbf{Noise Vulnerability} & Zero (Exact math) & Zero (CPU precision) & Exponential decay $\exp(-D p_e)$ & Mitigated ($86.46\%$ recovery) \\
...
\begin{itemize}
    ...
    \item \textbf{Implementation \& Evidence:} Realized in Section \ref{sec:hardware_noise_zne}, with code executed in \texttt{src/chapter\_4\_quantum/build\_and\_run\_03\_nisq\_zne.py}. An isolated 5-qubit inference kernel ($D = 39$ native gates) was subjected to calibrated superconducting noise profiles ($T_1 = 150\,\mu\mathrm{s}$, $T_2 = 120\,\mu\mathrm{s}$, $p_2 = 1.20\%$). Hardware decoherence degraded the unmitigated expectation value by $23.6\%$ ($0.0500 \to 0.0382$). By applying global unitary gate folding at scale factors $\lambda \in \{1, 3, 5\}$ and performing second-order Richardson extrapolation, the mitigated expectation value recovered to $E_{\mathrm{ZNE}} = 0.04840$, successfully canceling $86.46\%$ of hardware-induced noise without requiring physical qubit overhead.
    \item \textbf{Status:} \textbf{Fully Achieved.}
\end{itemize}
...
\begin{adjustbox}{max width=\textwidth}
\small
\begin{tabularx}{\textwidth}{>{\bfseries\raggedright\arraybackslash}p{3.2cm}>{\raggedright\arraybackslash}X>{\raggedright\arraybackslash}p{3.4cm}>{\raggedright\arraybackslash}p{2.6cm}c}
\toprule
\textbf{Thesis Objective} & \textbf{Methodological Target} & \textbf{Codebase Artifact} & \textbf{Validation Metric} & \textbf{Status} \\
\midrule
Primary Objective \newline (End-to-End Pipeline) & Hybrid QBN--QAE inference on exoplanet K2-18b with quadratic speedup & \texttt{src/chapter\_4\_quantum/} \newline \texttt{02\_QAE\_Ideal\_Simulation} & $\mathcal{O}(1/M)$ scaling, \newline $M \sim 10^4$ vs $10^8$ & \textbf{Verified} \\
\midrule
Sub-Goal 1 \newline (Astrobiological DAG) & 9-node causal DAG translated from JWST K2-18b spectra & \texttt{src/chapter\_2\_classical/} \newline \texttt{01\_Classical\_Limits} & Normalized CPTs, \newline 9 physical nodes & \textbf{Verified} \\
\midrule
Sub-Goal 2 \newline (17-Qubit Topology) & Register allocation: $n_E = 5$, $n_S = 9$, $n_A = 3$ qubits & \texttt{src/chapter\_4\_quantum/} \newline \texttt{build\_and\_run\_02} & $32$ phase bins, \newline 31 Grover iterates & \textbf{Verified} \\
\midrule
Sub-Goal 3 \newline (Unitary Compilation) & Pure subcircuits without barriers via \texttt{.to\_gate()}, $S_\chi$, $S_0$ & \texttt{src/chapter\_4\_quantum/} \newline \texttt{build\_and\_run\_02} & Invariant 2D rotation, \newline Zero barrier defects & \textbf{Verified} \\
\midrule
Sub-Goal 4 \newline (NISQ Noise \& ZNE) & Calibrated IBM noise simulation and ZNE error mitigation via Mitiq & \texttt{src/chapter\_4\_quantum/} \newline \texttt{03\_NISQ\_ZNE\_Mitigation} & $86.46\%$ noise recovery, \newline $\lambda \in \{1, 3, 5\}$ folding & \textbf{Verified} \\
\midrule
Sub-Goal 5 \newline (Classical Benchmark) & Empirical baseline in \texttt{pgmpy}: JT memory limits and MCMC variance & \texttt{src/chapter\_2\_classical/} \newline \texttt{build\_and\_run\_01} & $91.28\%$ rejection, \newline $\mathrm{RE} \to \infty$ at $p=10^{-6}$ & \textbf{Verified} \\
\bottomrule
\end{tabularx}
\end{adjustbox}
\end{table}
...
\notatfm{\textbf{Coherence Horizon:} ZNE mitigates $86.46\%$ of Lindblad decoherence on a 5-qubit kernel, but the 42,000-CNOT depth of canonical QAE necessitates fault tolerance.}
