...
Under finite-shot execution ($N_{\mathrm{shots}} = 2,048$), this induces the measurement distribution depicted in Figure \ref{fig:qae_ideal_spectrum}: $80.22\%$ of shots concentrate in $\ket{00000}$ ($y=0$), while $19.78\%$ appear at the complex conjugate Grover eigenvalue $\ket{10000}$ ($y=16$). This distribution closely tracks the analytical lower bound established by Brassard et al.~(2002) \cite{brassard2002}:
\begin{equation}
...
\begin{quote}
\emph{Hostile Tribunal Objection 2:} ``You report an $86.44\%$ noise error reduction using Zero-Noise Extrapolation (ZNE) on an isolated 5-qubit kernel of depth 39. Yet, the master 17-qubit canonical QAE circuit compiles to over 42,000 CNOT gates. Theoretical bounds established for quantum error mitigation \cite{temme2017, endo2018} prove that error mitigation without active error correction suffers from an exponential sampling overhead $\mathcal{O}(e^{2 \gamma D})$. Applying ZNE to a 42,000-gate circuit on contemporary hardware would drive the output into the maximally mixed state $I/2^N$, where Richardson extrapolation exponentially amplifies variance. Is it scientifically defensible to present ZNE as a bridge to near-term NISQ utility when a 42,000-gate circuit remains physically intractable?''
\end{quote}
