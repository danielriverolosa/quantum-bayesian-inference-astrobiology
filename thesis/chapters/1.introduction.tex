...
\begin{equation}
P(X_{\mathrm{target}} \mid \mathbf{E} = \mathbf{e}) = \frac{P(X_{\mathrm{target}}, \mathbf{E} = \mathbf{e})}{P(\mathbf{E} = \mathbf{e})} = \frac{\sum_{Y} \prod_{i=1}^N P\left(X_i \mid \mathrm{Parents}(X_i)\right)}{\sum_{X_{\mathrm{target}}} \sum_{Y} \prod_{i=1}^N P\left(X_i \mid \mathrm{Parents}(X_i)\right)},
...
\section{Methodological Scope and Theoretical Boundary Conditions}\label{sec:methodological_scope}

To establish the scientific rigor of this investigation and clearly delineate its operational domain across theoretical computer science, observational astrobiology, and quantum hardware engineering, this section establishes three foundational methodological boundaries:

\subsection{Complexity Scope: Bounded Acceleration vs. NP-Hardness}
A fundamental theoretical question in quantum complexity concerns whether asserting quantum acceleration for Bayesian inference contradicts the proven NP-hardness of probabilistic reasoning in graphical models \cite{cooper1990, dagum1993}. Because quantum algorithms cannot solve NP-complete problems in polynomial time ($\mathrm{BQP} \not\supseteq \mathrm{NP}$), establishing the exact computational regime where quantum advantage operates is critical.

This thesis makes no claim that Quantum Bayesian Networks reduce the worst-case NP-hard complexity of general Bayesian inference to polynomial time. In the worst case of a maximally connected DAG ($k_{\max} \to N$), the quantum state preparation complexity $\mathcal{O}(N \cdot 2^{k_{\max}})$ becomes exponential, mirroring the classical intractability. 
...
From an astrophysical modeling perspective, one must consider how discrete Bayesian networks interface with established continuous atmospheric retrieval tools (e.g., radiative transfer solvers such as \texttt{petitRADTRANS} or \texttt{NEMESIS}). Specifically, establishing why a discrete causal representation is adopted clarifies that it operates as a structured epistemic layer rather than an oversimplification of continuous atmospheric physics.

The Quantum Bayesian Network is not designed to replace high-resolution continuous radiative transfer solvers. Instead, it operates as a hierarchical epistemic synthesis layer positioned directly above continuous retrieval codes. Radiative transfer tools perform the low-level physical parameter extraction, converting continuous absorption cross-sections into posterior marginal distributions over gas abundances ($\mathrm{H}_2\mathrm{O}$, $\mathrm{CH}_4$, $\mathrm{CO}_2$). 
...
\subsection{Hardware Scope: NISQ Constraints and Error Mitigation Pathways}
From an experimental standpoint, contemporary Noisy Intermediate-Scale Quantum (NISQ) processors \cite{preskill2018} are constrained by physical two-qubit gate error rates ($\sim 10^{-2}$) and limited coherence times ($T_1, T_2$). Synthesizing multi-controlled gates and QAE on a 17-qubit circuit demands a rigorous appraisal of physical feasibility versus long-term fault-tolerant requirements.
...
The remainder of this Master's Thesis is structured into four specialized chapters followed by two technical appendices, sequentially bridging astrobiological theory, quantum computational mathematics, applied software engineering, and global scientific conclusions:
...
\begin{itemize}
    ...
    \item \textbf{Appendices A \& B: Exhaustive CPT Derivations and Transpilation Gate Inventories.} Appendix \ref{app:complete_cpts} documents the complete astrophysical parameterization and state spaces for all nine causal nodes. Appendix \ref{app:transpilation_profiles} provides the empirical transpilation metrics, gate breakdowns ($>53,000$ operations), and ZNE unitary folding scaling profiles.
\end{itemize}
