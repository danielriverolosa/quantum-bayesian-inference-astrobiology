...
\section{Methodological Scope and Theoretical Boundary Conditions}\label{sec:methodological_scope}\label{sec:preemptive_defense}
...
From an astrophysical modeling perspective, one must consider how discrete Bayesian networks interface with established continuous atmospheric retrieval tools (e.g., radiative transfer solvers such as \texttt{petitRADTRANS} or \texttt{NEMESIS}). Specifically, it is vital to clarify why a discrete causal representation is adopted rather than treating it as an oversimplification of a continuous atmosphere.
