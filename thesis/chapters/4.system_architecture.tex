...
\begin{quote}
\emph{Hostile Tribunal Objection 1:} ``In Figure \ref{fig:qae_ideal_spectrum}, simulating an ultra-rare technosignature ($a = 10^{-6}$) yields measurement counts concentrated in state $\ket{10000}$ ($y=16$, $100\%$). How can an examination tribunal accept that measuring state 16 proves a non-zero technosignature detection instead of a trivial outcome?''
\end{quote}
...
\begin{quote}
\emph{Hostile Tribunal Objection 2:} ``The master QAE circuit with 5 evaluation qubits has a transpiled depth exceeding 52,000 gates. No physical superconducting processor today can execute 52,000 gates within $T_1 \approx 150\,\mu\mathrm{s}$. Why claim your architecture is viable on NISQ hardware when you had to isolate an abbreviated 39-gate core to demonstrate ZNE?''
\end{quote}
