...
\begin{itemize}
    ...
    \item \textbf{MCMC Rare-State Trap:} In a Gibbs sampling simulation of $100,000$ steps (burn-in $2,000$, empirical transition acceptance rate $24.47\%$), the common biomarker DMS ($X_7$) was visited $1,926$ times ($P \approx 0.01926$), while the technosignature CFC ($X_8$) recorded zero visits ($0$ hits). In accordance with Kac's Return Theorem:
    \begin{equation}
...
\end{equation}
In finite-shot sampling ($N_{\mathrm{shots}} = 2,048$), this projects $80.22\%$ of the amplitude onto the nearest discrete state ($y=0$), while the Grover conjugate eigenvalue $\lambda_- = e^{-2i\theta_a}$ accounts for the remaining $19.78\%$ at $\ket{10000}$ ($y=16$). 
