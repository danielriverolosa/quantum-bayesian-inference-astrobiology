...
\subsection{Spectral Analysis and Eigenvalues of the Grover Operator \texorpdfstring{$\mathcal{Q}$}{Q}}
...
\begin{equation}
\ket{w_\pm} = \frac{1}{\sqrt{2}} \left( \ket{\Psi_0} \mp i \ket{\Psi_1} \right).
\label{eq:grover_eigenvectors_ch3}
\end{equation}
Expressing the initial state $\ket{\Psi}$ in this eigenbasis yields:
\begin{equation}
\ket{\Psi} = \frac{1}{\sqrt{2}} \left( e^{i\theta}\ket{w_+} + e^{-i\theta}\ket{w_-} \right).
\label{eq:state_expansion_eigenbasis_ch3}
\end{equation}
Equation \eqref{eq:state_expansion_eigenbasis_ch3} establishes that applying $\mathcal{Q}$ to the quantum system imparts distinct phase shifts of $\pm 2\theta$ to the eigencomponents ($\mathcal{Q}\ket{w_\pm} = e^{\pm i 2\theta}\ket{w_\pm}$). Measuring this phase shift directly reveals $\theta$, and consequently the posterior probability $a = \sin^2(\theta)$.
...
\end{equation}
where $S_M(\delta) = \sum_{k=0}^{M-1} e^{i 2\pi k \delta / M} = \frac{1 - e^{i 2\pi \delta}}{1 - e^{i 2\pi \delta / M}}$ is the geometric interference kernel that sharply concentrates probability amplitude around integers $y \approx \frac{M\theta}{\pi}$ and $y \approx M - \frac{M\theta}{\pi}$.
...
\end{equation}
Because the sine function satisfies reflection symmetry $\sin(\pi - \phi) = \sin(\phi)$, evaluating the estimator at the complementary conjugate peak $y_2 = M - y_1$ yields $\sin^2\left(\frac{\pi(M - y_1)}{M}\right) = \sin^2\left(\pi - \frac{\pi y_1}{M}\right) = \sin^2\left(\frac{\pi y_1}{M}\right) = \tilde{a}$. Consequently, the probability estimator is strictly invariant under the $\pm 2\theta$ phase ambiguity of the Grover eigenvalues, guaranteeing robust extraction of the target posterior from either spectral lobe. Figure \ref{fig:circuit_qae_master} shows the end-to-end master circuit architecture integrating the evaluation register, state register, Grover cascade, and IQFT block.
...
\section{Theoretical Boundary Conditions and Gate Synthesis Analysis}\label{sec:theoretical_boundaries_ch3}

To establish the mathematical rigor of this chapter and clearly delineate its theoretical foundation across quantum gate synthesis, calibration limits, and open-system noise dynamics, this section addresses three fundamental analytical considerations:

\subsection{Infinitesimal Rotation Angles vs. Physical Gate Calibration Tolerances}
An important experimental consideration is whether encoding an ultra-rare prior (such as a technosignature at $p \approx 10^{-6}$, requiring an individual rotation angle $\theta = 2\arcsin(\sqrt{10^{-6}}) \approx 0.002$ rad) is physically washed out by hardware noise, given that single-qubit gate calibration errors on physical superconducting devices are typically on the order of $\sim 10^{-3}$ to $10^{-2}$.

In a hierarchical Bayesian DAG, individual rare priors are never encoded in isolation. The prior probability of a technosignature is factorized conditionally: $P(\mathrm{CFC}=1) = P(\mathrm{CFC}=1 \mid \mathrm{Bio}=1) P(\mathrm{Bio}=1)$. Conditioned on an active biosphere, the local conditional rotation angle $\theta_{\mathrm{CFC} \mid \mathrm{Bio}}$ is substantial ($\sim 0.1$ to $0.3$ rad), well within native calibration tolerances. For non-biological branches, the angle is identically zero, requiring no rotation. Furthermore, QAE does not rely on resolving individual small rotation angles on single qubits; it amplifies the global geometric phase $2\theta$ through the global reflection operator $\mathcal{Q}$. Under ZNE mitigation, systematic over-rotations and depolarizing errors are scaled and canceled, preserving the target amplitude signal.

\subsection{Multi-Controlled Gate Synthesis and Treewidth In-Degree Bounds}
A second theoretical question concerns gate synthesis scaling: decomposing an ancilla-free $k$-controlled rotation $C^k(R_y)$ requires $\mathcal{O}(2^k)$ CNOT gates (Barenco et al., 1995 \cite{barenco1995}). If a complex planetary network were to contain nodes with $k=5$ or $6$ parents, the circuit depth could explode exponentially, potentially undermining the spatial compression advantage.

The topological architecture of exoplanetary atmospheres is naturally bounded. In realistic photochemical networks, individual atmospheric species are directly influenced by a sparse set of immediate parents ($k_{\max} \le 3$), keeping $2^{k_{\max}} \le 8$ CNOTs per conditional branch. For scaled networks where $k \ge 4$, our architecture utilizes borrowed dirty ancilla qubits: by borrowing idle evaluation qubits ($q_0 \dots q_4$) during the state preparation phase (which remain in their ground state $\ket{0}^{\otimes m}$ prior to the Hadamard initialization of the evaluation register), $C^k(R_y)$ operations can be decomposed with strictly linear gate complexity $\mathcal{O}(k)$ without allocating dedicated physical ancillas, fully maintaining polynomial circuit depth.

\subsection{Physical Validity of Zero-Noise Extrapolation under Coherent Noise and Drift}
A third analytical consideration involves the physical validity of Zero-Noise Extrapolation (ZNE). ZNE assumes that hardware noise scales monotonically and that gate folding $U \to U U^\dagger U$ preserves the underlying noise model. In real superconducting QPUs, coherent errors, crosstalk, and non-Markovian drifts could potentially violate the polynomial Taylor expansion in Equation \eqref{eq:zne_taylor_expansion_ch3}.

While coherent errors can induce oscillatory noise behavior, our implementation deploys randomized folding: gate folding identities are interleaved across different cycles and paired with randomized compiling (Pauli twirling). This twirling twirls coherent unitary errors into effective stochastic Pauli channels, validating the polynomial Taylor expansion. Furthermore, digital gate-level folding (implemented via Mitiq) ensures that quantum operations remain strictly within the computational subspace without inducing pulse-level distortions or transmon leakage to state $\ket{2}$, validating that the effective Lindbladian generator scales linearly as $\mathcal{L}_{\mathrm{eff}}(\lambda) \approx \lambda \mathcal{L}_0$ in the weak-coupling regime. Our pipeline evaluates three distinct extrapolation models (linear, polynomial, and Richardson) across conservative scale factors ($\lambda \in \{1, 3, 5\}$). As empirically proven in Chapter \ref{chap:system_architecture}, the mitigated expectation values converge within a verified error corridor, successfully recovering the theoretical signal under realistic IBM Quantum noise profiles.
