...
\end{equation}
Equation \eqref{eq:state_expansion_eigenbasis_ch3} establishes that applying $\mathcal{Q}$ to the quantum system imparts distinct phase shifts of $\mp 2\theta$ to the eigencomponents ($\mathcal{Q}\ket{w_\pm} = e^{\mp i 2\theta}\ket{w_\pm}$). Measuring this phase shift directly reveals $\theta$, and consequently the posterior probability $a = \sin^2(\theta)$.
...
\section{Theoretical Boundary Conditions and Gate Synthesis Analysis}\label{sec:theoretical_boundaries_ch3}\label{sec:preemptive_defense_ch3}
